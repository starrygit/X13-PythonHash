...
#if PY_MAJOR_VERSION >= 3
    x12sm3_hash((char *)PyBytes_AsString((PyObject*) input), output);
#else
    x12sm3_hash((char *)PyString_AsString((PyObject*) input), output);
#endif
    Py_DECREF(input);
#if PY_MAJOR_VERSION >= 3
...
PyMODINIT_FUNC PyInit_x12sm3_hash(void) {
    return PyModule_Create(&X13Module);
}

#else

PyMODINIT_FUNC initx12sm3_hash(void) {
    (void) Py_InitModule("x12sm3_hash", X13Methods);
}
#endif
